...
train lenet on digit classification task training set.
no data augmentation, no invariance at this point.
give accuracy, in the expected results \cite{mnist_web} (LeNet-5 is 0.95 error rate).
take model, remove layers for classification (removing softmax): softmax and last inner-product layer (10 outputs).
last remaining layer is ReLU after inner-product, 500 dimensions.
hypothesis: this last layer should contains a high-level fingerprint of digits useful for classification.
we apply pca followed by t-sne on test set.
digits clearly separated, one cluster per digit.
the structure shows our hypothesis seems correct, separation of class, coherence of similarity.
most distortions are part of clusters but extremes are outside.

apply the experiment again but learn translation invariance.
one simple way is data augmentation to train model to recognize digits when translated.
we can compare to data-augmented model for translation.
give accuracy, show it is translation invariant.
hypothesis: the fingerprint now should also contains many features of digits with translation.
translation is dominating the clustering.
inside each cluster, visible digit separation.
digits are heavily overlapping, not easily separable.
...
lenet on mnist without data-augmentation easily fooled with distortion.
as said, mnist is heavily processed, nearly no variance left in dataset to train model.
however small rotations works well because it's naturally present.
adding multiplies translated copies in dataset greatly reduce errors on distorted.
however t-sne embedding is ``overcrowded'' when augmented due to fuzzy boundaries between clusters.
cause is due to important energy change in pixel space, \eg: center of mass is translated.
network probably learned to separate the most important source of variance and acquired position-specific features to distinguish translated digits which impacts fingerprints of the last layer.
t-SNE used this primary clustering energy for translation, then digits as its second major structure.
it represents hierarchal structures with decreasing importance due to its optimization formulation, discriminative features.

unexpected clustering.
recall tsne is for visualization.
wanted to create invariant-model for classification and decode the fingerprint with t-sne to create a simple coherent plot where class and distortion would be cleanly separated into different axises.
however as shown above t-sne doesn't work that way.
its formulation doesn't allow to tweak it for a particular goal.
cannot give easily hints about number of variance dimensions to represent.
the resulting embedding is confusing for humans.
this is due to lack of control
need supervised method where prior knowledge can be used to guide it.

in the following we present the results of our alternative method.

...
\begin{abstract}
    ...
    In this work, insights are provided about CNN embeddings and propose a new loss function, derived from the contrastive loss, that creates models with more predicable mappings and also quantifies distortions.
    In typical distortion-dependent methods, there is no simple relation between the features corresponding to one image and the features of the distorted image.
    Therefore, these methods require to feed-forward inputs under every distortions in order to find the corresponding features representation.
    ...
    A simple method is introduced to compare quantitatively embeddings of models which are used for dimensionality reduction.
\end{abstract}
...
There is an enormous flow of information in terms of quantity and dimensionality.
...
% FIXME: continue conversion: familiar -> neutral

...
To compare with the two DrLIM's results, we train for each one a network in the original configurations and an other network with the extension as described in the methodology section for two-label pairs.

The first dataset is a subset containing only 4s and 9s digits and we derive the training and testing sets based on the pairing strategy of the respective model.
show plot embedding on test set. see figure \ref{fig:mnist_cl2d}.
...
\begin{figure}[h]
    ...
    \end{center}
    \caption{Plot of the embedding of our network on the MNIST data-augmented test set.}

...
Ways to speed up the training and the classification appeared and were greatly beneficial for the development of this field in recent years\cite{ciresan2011flexible}\cite{schmidhuber2015deep}\cite{nasse2009face}. % FIXME: more citations
...
%Moreover the input-output relation, when inputs are kept distorted on purpose, is usually either killing the signal (invariance) or not in a predictable manner (controlled preservation). % FIXME: keep this sentence for later?
...
% for siamese
In order to compare our results to our reference paper, we trained two Siamese models replicating their settings on MNIST and NORB.
...
One important advantage is to allow a more control, especially in how dimensions are used. using a single model.
...
As previously said, the dimensions of $p$ and the set $D$ needs to be defined considering our goal.

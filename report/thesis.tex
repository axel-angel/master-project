...
Most of our experiments were implemented using the Python programming language: to prepare datasets, to compute measures and project for visualization.
...
All this functionalities give us the necessary tools to reasonably inspect the results and to come with the insights we needed.
...
However it is still a good subject of experiments to try and validate new ideas easily.
...
The {\bf NORB} dataset ({\em NYU Object Recognition Benchmark}) is a collection of photos of toy figures taken from continuous poses.
...
It is composed of $48'600$ processed photos of toy, as illustrated by figure~\ref{fig:norb}.
...
We will also use this toy to make our results because we want to compare with their work.
...
% FIXME: continue correction from here
...
That is why we propose a very simple quantitative measure that allows us to compare DrLIM and our models.
...
% FIXME: moved here, need to merge
In this experiment, we are generating a classification task where the training dataset contains one untouched and multiple translated digits with the corresponding label.
...
\begin{figure}[t]
    ...
    We can see qualitatively that most of the original digits are well separated into clusters for each class.
    The major exception are the 1s spreading on a thin vertical curve over a small part of he 6s.
    Inside clusters, digits are spread coherently to their features.
    }
...
\begin{figure*}[h]
    ...
    The result is a thin cylinder whose long axis represent the elevation and the radius is the azimuth angle.
    However some parts are flat along the elevation axis and the radius is fuzzy.
    }
    % FIXME: add why it is nice
...
We moved to solutions based on NN because they have many advantages, for example that they can create very complex embeddings and a functional mapping to it.
...
However regression is more constrained than the current formulation because: (1) clusters can be moved around, only their relative positions matter whereas their regression counterpart are forced to absolute positions (2) the margin formulation allows to put extra gap to make space between clusters without penalization (like SVM) whereas regression create a cost in every directions.
...
For instance by using the produced features in a bigger classification network, for MNIST: gives the distortion intensity and the digit class; for NORB: the pair of azimuth-elevation angles.
...
% FIXME: rephrase, explicit
A better experiment is to train a model to compute all distorted inputs' features from a single input's features where the accuracy of such a model would serve as a score.
...
It is important to understand the normalization step to extract features like the rotation is costly especially for 3D images and even specialized FPGAs were designed to compute 3D expensive rotations.
